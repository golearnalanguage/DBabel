...
window.installWorkbenchViews = function(ctx) {
  ...
  const notice=node('div','','workbench-notice');notice.setAttribute('role','status');notice.hidden=true;document.body.append(notice);
  let noticeTimer;
  ...
  function button(label,action){const b=node('button',label,'view-action');b.type='button';b.addEventListener('click',action);return b;}
  function start(title,description){view.replaceChildren(node('p','DBABEL / WORKSPACE','eyebrow'),node('h1',title),node('p',description,'view-description'));}
  function card(title,text){const c=node('article',undefined,'summary-card');c.append(node('h2',title),node('p',text));view.append(c);return c;}
  function openUnit(u){show('Review');selectUnit(u.id);el('inspector').scrollIntoView({block:'nearest'});}
  function unitCard(u,detail){const c=card(u.location||u.id,detail);c.append(button('Review '+u.id,()=>openUnit(u)));return c;}
  ...
  function show(name){
    ...
    if(name==='Terminology'){
      ...
      for(const e of state.evidence){const c=card(e.source_title||e.id,e.support_note||'No support note supplied.');c.append(node('p',e.locator||'No locator','evidence-locator'));
        const raw=e.url||e.locator||'';if(/^https?:\/\//i.test(raw)){const a=node('a','Open source','view-action');a.href=raw;a.target='_blank';a.rel='noopener noreferrer';c.append(a);}
      ...
      card('Export capability',getReport?(state.exportAvailable?'Native DOCX export configured.':'Review only. Start with --original and --output for native DOCX export.'):'Offline decisions only. Import and recheck in a local session before native export.');
