(() => {
  ...
  const transferKey='dbabel-language-transfer:'+state.session.session_id;let languageSelection=null;
  try{const saved=sessionStorage.getItem(transferKey);if(saved){const snapshot=JSON.parse(saved);for(const d of snapshot.decisions)if(state.decisions.has(d.unit_id))state.decisions.set(d.unit_id,d);if(state.units.some(u=>u.id===snapshot.selected))languageSelection=snapshot.selected;sessionStorage.removeItem(transferKey);}}catch(_){}
  window.prepareLanguageReload=()=>{try{sessionStorage.setItem(transferKey,JSON.stringify({decisions:[...state.decisions.values()],selected:state.selected}));return true;}catch(_){window.workbenchNotice(trUI('Download decisions before switching language; browser storage is unavailable.'));return false;}};
  window.hasUnsavedWorkbenchChanges=()=>{if(!state.selected)return false;const u=state.units.find(x=>x.id===state.selected),d=decisionFor(state.selected);return (state.editing&&el('targetText').value!==(d.approved_target??u.current_target))||el('reviewerNote').value!==(d.reviewer_note||'');};
  ...
  let indexedIssues=null,issueIndex=new Map();
  function issuesFor(id){if(indexedIssues!==state.issues){indexedIssues=state.issues;issueIndex=new Map();for(const issue of state.issues){if(!issueIndex.has(issue.unit_id))issueIndex.set(issue.unit_id,[]);issueIndex.get(issue.unit_id).push(issue);}}return issueIndex.get(id)||[];}
  ...
  let indexedUnits=null,unitIndexes=new Map();function unitIndex(id){if(indexedUnits!==state.units){indexedUnits=state.units;unitIndexes=new Map(state.units.map((u,i)=>[u.id,i]));}return unitIndexes.get(id)||0;}
  ...
  function searchable(u){
    return [
      ...
      u.source,
      u.current_target,
      u.suggested_target||'',decisionFor(u.id).approved_target||'',
      ...(u.labels||[]),
  ...
  function renderIssueFilters(){const root=el('issueFilterList');root.replaceChildren();for(const [key,label,color] of issueFilterDefs){const count=state.units.filter(u=>issueFilterMatch(u,key)).length;if(!count)continue;const row=node('label',undefined,'filter-row'),input=node('input');input.type='checkbox';input.checked=state.issueFilters.has(key);input.addEventListener('change',()=>{input.checked?state.issueFilters.add(key):state.issueFilters.delete(key);state.selectedUnits.clear();state.page=0;renderTable();});const dot=node('span',undefined,'dot '+color),labelNode=node('span',trUI(label));labelNode.title=key.endsWith('_GROUP')?label:key;row.append(input,dot,labelNode,node('b',count));root.append(row);}}
  ...
  function renderTable(){
    ...
    el('pageInfo').textContent=
      items.length
        ? trUI(`${start+1}–${end} of ${items.length.toLocaleString()}`)
        : trUI('0–0 of 0');
  ...
  function renderSuggestion(u){
    ...
    el('reasonBox').textContent=advice.reason;
  ...
  function renderTechniqueMetadata(u){
    ...
    if(!annotated.length){
      root.append(
        node(
          'div',
          trUI('No technique annotation is attached to this finding.'),
          'technique-empty'
        )
      );
      return;
  ...
  function renderSuggestionEvidence(u){const evs=evidenceFor(u),root=el('suggestionEvidenceList');root.replaceChildren();el('suggestionEvidenceCount').textContent=evs.length?trUI(`View all (${evs.length})`):trUI('No linked evidence');for(const e of evs.slice(0,1)){const c=node('div',undefined,'evidence-card');c.append(node('div',e.source_title||e.id,'evidence-title'));c.append(node('div',e.support_note||'','evidence-note'));c.append(node('div',e.locator||'','evidence-locator'));root.append(c);}if(!evs.length)root.append(node('div',trUI('No evidence record is linked to this unit.'),'evidence-card'));}
  ...
  function editTarget(){state.editing=!state.editing;el('targetText').readOnly=!state.editing;el('targetText').classList.toggle('editing',state.editing);el('editAction').textContent=state.editing?'Use Edit':'Edit';if(state.editing)el('targetText').focus();}
